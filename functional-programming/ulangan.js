...
function getProductsByCategory(products, category) {
   ...
   */
  return [...products].filter((variable) => variable.category === category);
}
...
function findProductById(products, id) {
   ...
   */
  return [...products].filter((variable) => variable.id === id);
}
...
function calculateTotalPrice(products) {
   ...
   */
  return [...products].reduce((arr, prices) => arr + prices.price, 0);
}
...
function applyDiscount(products, discount) {
   ...
   */
  return [...products].map(
    (disc) => disc.price - (discount / 100) * disc.price
  );
}
